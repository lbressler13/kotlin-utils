...
fun runFilterToConstSetTests() {
    var intSet = emptyConstMultiSet<Int>()
    var intExpected = emptyConstMultiSet<Int>()
    assertEquals(intExpected, intSet.filterToConstSet { true })
    ...
    assertEquals(intExpected, intSet.filterToConstSet { false })

    intExpected = constMultiSetOf(1, 1, 1, 5, 5, 0, 0)
    assertEquals(intExpected, intSet.filterToConstSet { intSet.getCountOf(it) > 1 })

    intExpected = constMultiSetOf(1, 1, 1, 3, -1, 6)
    val testInts = setOf(-1, 1, 11, 12, 10, 3, 6, -2)
    assertEquals(intExpected, intSet.filterToConstSet { it in testInts })
    ...
    assertEquals(stringExpected, stringSet.filterToConstSet { it.length in 3..5 })
    ...
    assertEquals(errorExpected, errorSet.filterToConstSet { it is ClassCastException })

    // modified
    intSet = constMultiSetOf(1, 1, 2, 14, 14)
    intExpected = constMultiSetOf(1, 2, 14, 14)
    ...
    val listSet: ConstMultiSet<IntList> = constMultiSetOf(listOf(1, 2, 3), listOf(0, 5, 7))
    val listExpected = constMultiSetOf(listOf(1, 2, 3))
    assertEquals(listExpected, listSet.filterToConstSet { it.contains(2) })
}
