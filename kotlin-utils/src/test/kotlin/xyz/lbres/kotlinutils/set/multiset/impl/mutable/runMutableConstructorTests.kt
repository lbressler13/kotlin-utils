...
fun runMutableConstructorTests() {
    var set: MutableMultiSet<Int> = MutableStandardMultiSet(emptyList())
    var expectedSize = 0
    var expectedDistinct = emptySet<Int>()
    assertEquals(expectedSize, set.size)
    assertEquals(expectedDistinct, set.distinctValues)

    set = MutableStandardMultiSet(setOf(-12))
    expectedSize = 1
    expectedDistinct = setOf(-12)
    assertEquals(expectedSize, set.size)
    assertEquals(expectedDistinct, set.distinctValues)

    set = MutableStandardMultiSet(mutableListOf(10, 10, 10, 10))
    expectedSize = 4
    expectedDistinct = setOf(10)
    assertEquals(expectedSize, set.size)
    assertEquals(expectedDistinct, set.distinctValues)

    set = MutableStandardMultiSet(listOf(-12, 18, 4, 10000, 25, 25, -1, 0, 5, 25))
    expectedSize = 10
    ...
    val errSet = MutableStandardMultiSet(mutableSetOf(e1, e2, e3))
    expectedSize = 3
    ...
    val listSet = MutableStandardMultiSet(listOf(listOf(1, 3, 4), listOf(55, 66, 77)))
    expectedSize = 2
    ...
    val compListSet = MutableStandardMultiSet(listOf(listOf(1, 2, 3), listOf("abc", "def"), listOf("abc", "def")))
    expectedSize = 3
